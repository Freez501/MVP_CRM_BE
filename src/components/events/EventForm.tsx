...
export function EventForm({ isOpen, onClose, onSubmit }: EventFormProps) {
  ...
  const Label = ({ children, htmlFor }: { children: React.ReactNode; htmlFor?: string }) => (
    <label htmlFor={htmlFor} className="block font-montserrat font-light uppercase tracking-[0.12em] text-xs text-text-tertiary mb-1.5">
      {children}
    </label>
  )
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <Select id="client" value={form.clientId} onChange={(e) => setForm({ ...form, clientId: e.target.value })} required>
            ...
            <div>
              ...
              <Input id="bartenders" type="number" min={1} value={form.bartendersCount} onChange={(e) => setForm({ ...form, bartendersCount: Number(e.target.value) })} />
            </div>
            <div>
            ...
            <div>
              ...
              <Input id="clothing" value={form.clothing} onChange={(e) => setForm({ ...form, clothing: e.target.value })} placeholder="Например, черная рубашка" />
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
            {[
              ...
              <div key={key}>
                ...
                <Input id={key} type="time" value={(form as any)[key]} onChange={(e) => setForm({ ...form, [key]: e.target.value } as any)} />
              </div>
            ))}
          </div>
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <Select id="bar" value={form.bar} onChange={(e) => setForm({ ...form, bar: e.target.value })}>
              ...
              <div>
                ...
                <Input id="barComment" value={form.barComment} onChange={(e) => setForm({ ...form, barComment: e.target.value })} />
              </div>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <Label htmlFor="shelf">Стеллаж</Label>
                <Select id="shelf" value={form.shelf} onChange={(e) => setForm({ ...form, shelf: e.target.value })}>
              ...
              <div>
                ...
                <Input id="shelfComment" value={form.shelfComment} onChange={(e) => setForm({ ...form, shelfComment: e.target.value })} />
              </div>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <Label htmlFor="pyramid">Пирамида</Label>
                <Select id="pyramid" value={form.pyramid} onChange={(e) => setForm({ ...form, pyramid: e.target.value })}>
              ...
              <div>
                ...
                <Input id="pyramidComment" value={form.pyramidComment} onChange={(e) => setForm({ ...form, pyramidComment: e.target.value })} />
              </div>
            </div>
          </div>
          ...
          <div className="mt-4">
            ...
            <Input id="decorationComment" value={form.decorationComment} onChange={(e) => setForm({ ...form, decorationComment: e.target.value })} />
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <Select id="menu" value={form.menu} onChange={(e) => setForm({ ...form, menu: e.target.value })}>
              ...
              return (
                ...
                >
                  <div>
                    ...
                    <p className="font-montserrat text-xs text-text-tertiary">{cocktail.category}</p>
                  </div>
                  <div className="flex items-center gap-3">
                    ...
                    <span className="w-6 text-center font-montserrat text-text-primary">{selected ? selected.qty : 0}</span>
                    <button
                      type="button"
                      onClick={() => updateCocktailQty(cocktail.name, 1)}
                      className="w-8 h-8 rounded-full border border-border-sketch flex items-center justify-center text-text-secondary hover:border-brand hover:text-text-primary transition-colors"
